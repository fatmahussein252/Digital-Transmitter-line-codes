...
%   This function computes the mean of a vector
    function m=MEAN(data,num_realizations)
        sum=0;
        for i=1:num_realizations
            sum=data(i)+sum;
        end
        m=sum/num_realizations;
    end
% Description:
...
%   code signal.
%   The resulting plot is titled with the provided string 'str'.
    function time_autocorrelation(ensemble, str, num_samples)
        
          % Pre-allocate the autocorrelation matrix
        Rx = zeros(1, num_samples);
      
           % Loop through time for the first realization
        for tau=0:num_samples-1
            sum=0;
            for n=1:num_samples-tau
                R=ensemble(1,n)*ensemble(1,n+tau);
                sum=sum+R;
            end
           Rx(1,tau+1)=sum/num_samples;
        end
 Rx = [fliplr(Rx) Rx];%flip the matrix and concatenate , as Rx is even function
 Rx = cat(2,Rx(1,1:num_samples-1),Rx(1,num_samples+1:2*num_samples));%remove Rx(0) as it repeated twice
        time = -699:699;
        subplot(2,1,2);
        plot(time,Rx);
        axis([-num_samples+1 num_samples-1 -3 20])
        title(str);
        ylabel('magnitude');
        xlabel('tau');
    end
...
%   This function contains the main code logic that will be executed.
    ...
    function main()
        ...
        time_autocorrelation(ensemble, ("time Autocorrelation - polar NRZ line code"), num_samples);
        ...
        time_autocorrelation(ensemble, ("time Autocorrelation - Unipolar line code"), num_samples);
        ...
        time_autocorrelation(ensemble, ("time Autocorrelation - polar RZ line code"), num_samples);
